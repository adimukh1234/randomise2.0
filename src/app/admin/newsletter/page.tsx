...
export default function NewsletterComingSoonPage() {
  ...
  // Enhanced strict regex to catch more fake emails
  ...
  const handleSubmit = useCallback(
    async (e) => {
      ...
      // 1. Honeypot check (Catches dumb bots)
      if (honeypot) { 
        setStatus("success"); 
      ...
      // 3. Anti-Spam Rate Limiter (1 submission per minute per browser)
      const lastSubmit = localStorage.getItem('newsletter_last_submit');
      if (lastSubmit && Date.now() - parseInt(lastSubmit) < 60000) {
        setStatus("error");
        setMessage("Please wait a minute before subscribing again.");
        return;
      }
      ...
      try {
        ...
        // Mark successful submission in local storage for rate limiting
